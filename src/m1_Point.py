...
class Point(object):
    ...
    def move_to(self, x, y):
        self.dist = self.dist + abs(self.x - x) + abs(self.y - y)
        self.x = x
        self.y = y
    ...
    def move_by(self, dx, dy):
        ...
        self.y = self.y + dy
        self.count = self.count + 1
        self.dist = self.dist + ((self.x - (self.x + dx)) ** 2 + (self.y - (self.y + dy)) ** 2) ** 0.5
    ...
    def closer_to(self, p2, p3):
        p2dist = ((self.x - p2.x) ** 2 + (self.y - p2.y) ** 2) ** 0.5
        p3dist = ((self.x - p3.x) ** 2 + (self.y - p3.y) ** 2) ** 0.5
        if p2dist >= p3dist:
            return p2
        else:
            return p3
    ...
    def halfway_to(self, p2):
        p3 = Point(0, 0)
        if self.x <= p2.x:
            p3.x = abs(p2.x - self.x) / 2 + self.x
        if self.y <= p2.y:
            p3.y = abs(p2.y - self.y) / 2 + self.y
        if self.y > p2.y:
            p3.y = -abs(p2.y - self.y) / 2 + self.y
        if self.x > p2.x:
            p3.x = -abs(p2.x - self.x) / 2 + self.x
